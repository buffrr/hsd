...
class RootServer extends DNSServer {
  ...
  async response(req, rinfo) {
    ...
    if (name === '.') {
      ...
      switch (type) {
        ...
        default:
          ...
          res.authority.push(createNSEC(".", nextName("."), TYPE_MAP_ROOT));
    ...
    if (!data) {
      ...
      if (item && item.root) {
        ...
        if (res.ad && res.code !== codes.NXDOMAIN) {
          ...
          if (ds.length === 0) {
            ...
            res.authority.push(createNSEC(tld, nextName(tld), TYPE_MAP_INSECURE));
            key.signZSK(res.authority, types.NSEC);
          } else {
      ...
      // NSEC record (white lies)
      // https://tools.ietf.org/html/rfc4470

      // Proving the name doesn't exist
      const nameNSEC = [ toNXDOMAIN(tld) ];
      key.signZSK(nameNSEC, types.NSEC);
      // Proving a wildcard doesn't exist
      const wildcardNSEC = [ toNXDOMAIN("*.") ];
      key.signZSK(wildcardNSEC, types.NSEC);

      res.authority = res.authority.concat(nameNSEC, wildcardNSEC);
      res.authority.push(this.toSOA());
      key.signZSK(res.authority, types.SOA);
...
// creates a minimally covering NSEC range
function toNXDOMAIN(name) {
  const prev = prevName(name);
  const next = nextName(name);

  return createNSEC(prev, next, TYPE_MAP_NXDOMAIN);
}

function createNSEC(name, nextDomain, typeBitmap) {
  const rr = new Record();
...
// find the successor of a name
function nextName(name) {
  return util.trimFQDN(name) + "\\000.";
}

// find the predecessor of a name
function prevName(name) {
  assert(name.length !== 0);

  let prev = Buffer.from(util.trimFQDN(name), "ascii");
  prev[prev.length-1]--;
  prev = prev.toString();
